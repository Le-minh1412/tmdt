...
product.getProductFeedbacks = async (product_variant_id) => {
  ...
  let getProductFeedbacks = `SELECT * FROM view_product_feedbacks WHERE product_id = ? AND feedback_is_display = 1 ORDER BY feedback_date DESC, feedback_id DESC`;

  return new Promise((resolve, reject) => {
    db.query(getProductFeedbacks, [product_id], (err, productFeedbacks) => {
      if (err) {
        console.log(err);
        resolve(0);
      } else {
